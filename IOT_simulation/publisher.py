    ...
    def veri_uret(self):
        ...
        return {
            ...
            "values": {
                ...
                "isik":        round(random.uniform(*profil["isik_ideal"]) + random.gauss(0, 200), 1),
...
try:
    while True:
        ...
        isik   = veri["values"]["isik"]
        sulama = "💧 AÇIK" if veri["values"]["sulama_aktif"] else "⏹  KAPALI"

        print(f"[{veri['timestamp']}] 🌱{veri['bitki']:8} | Nem:{nem:5.1f}% | Sıcaklık:{sicak:5.1f}°C | Işık:{isik:7.0f}lx | Sulama:{sulama}")
        time.sleep(PERIYOT)
