...
def db_oku():
    ...
    rows = [(v.get("toprak_nemi"), v.get("sicaklik"), v.get("isik"), v.get("sulama_aktif"), v.get("timestamp")) for v in veriler]
    return rows
...
def grafik_ciz():
    ...
    isiklar  = [r[2] for r in rows]
    sulamalar= [r[3] for r in rows]
    ...
    gs = gridspec.GridSpec(3, 2, width_ratios=[3, 1], hspace=0.55, wspace=0.25,
...
def on_message(client, userdata, msg):
    try:
        ...
        print(f"[{veri['timestamp']}] 🌿{veri['bitki']:8} | Nem:{nem:5.1f}% | Sıcak:{sicak:5.1f}°C | Işık:{isik:7.0f}lx | {sul}")
